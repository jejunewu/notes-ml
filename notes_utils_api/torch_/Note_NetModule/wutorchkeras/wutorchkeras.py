...
from wutorchkeras.summary import summary
...
class Model(torch.nn.Module):
    ...
    def fit(self, epochs, dl_train, dl_val=None, log_step_freq=999, log_epoch_freq=1):

        print(Model.print_bar()+"Start Training ...")
        ...
        for epoch in range(1, epochs + 1):
            ...
            for features, labels in dl_train:
                ...
                if step % log_step_freq == 0:
                    step_logs = Model.print_bar() + 'epoch={}/{}, '.format(epoch, epochs) + 'step={}/{}, '.format(step, len(dl_train))
                    logs = {}
                    logs.update({k: round(v / step, 3) for k, v in train_metrics_sum.items()})
                    for key in logs:
                        step_logs +=  key+'={}, '.format(logs.get(key))
                    print(step_logs)
            ...
            if epoch % log_epoch_freq == 0:
                epoch_logs = Model.print_bar()+'epoch={}/{}, '.format(epoch, epochs)
                ...
                # Model.print_bar()
                for key in infos:
                    epoch_logs +=  key+'={}, '.format(infos.get(key))

                print(epoch_logs)


        endTime = datetime.datetime.now()
        print(Model.print_bar()+"Finished Training...")
